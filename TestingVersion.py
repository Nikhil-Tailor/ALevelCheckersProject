...
class StartPage(tk.Frame):#This class uses the other class frame in order to show the tkinter images
    ...
    def __init__(self,parent,controller):
        ...
        self.img = tk.PhotoImage(file="circleW.png")##This sets the image file to be used as a variable
    ...
    def restart(self,app):#This is the restart function that sets variables back to an original value
    ...
    def updateClock(self):#This is the clock function that uses the original time and the current time to display the taken in the game
        ...
        mins=total//60#Uses DIV to calculate the amount of total minutes in taken in the game
        sec=total%60#Uses modulus to seconds part of the time
        txt=("\t\t"+str(mins)+" Minutes  "+str(sec)+" Secconds\t\t\t\t")
        #total=now-self.start
        self.label3.configure(text=txt)
        self.after(100, self.updateClock)#This repeats the function so that is keeps displaying the current time taken
        return mins,sec

    def database(self,mins,sec):#This function inputs data into the database
        mins=str(mins)
        sec=str(sec)
        time=mins+" mins "+sec+" sec "

        with sqlite3.connect ("CheckersTesting.db") as db:#Selects the maximum gameID in order to find the next gameID keeping it a unique primary key
            cursor=db.cursor()
    ...
    def showdata(self):#This function prints data from the database
        ...
        with sqlite3.connect ("CheckersTesting.db") as db:
            cursor=db.cursor()
            cursor.execute("SELECT ?, Score FROM Result",data)#Shows all the scores from player 2
            db.commit()
            result = cursor.fetchall()
        ...
        with sqlite3.connect ("CheckersTesting.db") as db:
            cursor=db.cursor()
            cursor.execute("select Resulta.GameID , Resulta.UserID as 'Winner',Resultb.UserID  as 'Looser',Game.Date,Game.Minutes,Game.Secconds from Result Resulta, Result Resultb INNER JOIN Game ON Resulta.GameID  = Game.GameID where Resulta.GameID=Resultb.GameID and Resulta.Score='Won' and Resultb.Score='Lost'")
            db.commit()
            result = cursor.fetchall()
        result.insert(0,prt)
        for i in result:
    ...
    def coListf(self,letter,x,y,maxx,newx,newy,xchange,xnew2,ynew2):#This uses nested for loops to make lists to show represent when the game pieces start
        c=0
        c1=0
        l=[]
        for j in range(12):
    ...
    def findCo(self,event):#This takes the x and y coordinates of where the user has clicked and uses DIV to calulate where on a grid they clicked
            ...
            xc=(1+(xcoordinate//100))#Uses integer division to calculate the position in a grid from a coordinate
            self.xlist.append(xc)#Adds the x position to a running list of every square the user has clicked


            yc=(1+(ycoordinate//100))
            self.ylist.append(yc)#Adds the y position to a running list of every square the user has clicked
            ...
            self.testmove()#Starts the testmove function

    def testmove(self):
        print(self.xlist)
        #This function determines if the user has made a valid more, using the list of all coordinates click and what to next


        label = tk.Label(self, text=self.player, font=FONT)
        validmove=False
#This looks at the previous two positions click and determines if it can use it to make a move

        if self.player ==1:
            coList=self.coList1

            if self.t==True and ((self.xlist[-2])-(self.xlist[-1])==-2) and ((self.ylist[-2])-(self.ylist[-1])==2):
                #Checks if a piece can go up and right by two spaces, stealing the other player piece
                self.move(1,2,-2,200,-200)

            elif self.t==True and ((self.xlist[-2])-(self.xlist[-1])==2) and ((self.ylist[-2])-(self.ylist[-1])==2):
                self.move(1,-2,-2,-200,-200)
                #Checks if a piece can go up and left by two spaces, stealing the other player piece
                ...
                #Checks if a piece can go down and left by two spaces, stealing the other player piece


            elif self.king==True and self.t==True and ((self.xlist[-2])-(self.xlist[-1])==-2) and ((self.ylist[-2])-(self.ylist[-1])==-2):
                self.move(2,2,2,200,200) 
                #Checks if a piece can go down and right by two spaces, stealing the other player piece
        ...
        if empty==False:#This checks if the user clicked on a piece, if they have it sets the variable "t" to true meaning it the next move could be valid
            for i in coList:
    ...
    def move(self,player,testx,testy,xmove,ymove):#Makes the move if it is valid
        ...
        currentposx=self.xlist[-2]#Sets the current positions as a variable
    ...
    def imagefunction(self,currentpiece):#Function to determine which piece image should move
...
def enterpw(username,password,player,message):
    ...
    #Selects password from the database to check if the password if correct
    with sqlite3.connect ("CheckersTesting.db") as db:
        cursor=db.cursor()
    ...
    if (valid==True) and (len(user)==0 or len(user)==1):
        ...
        elif len(user)==2:#If both users have entered the password correctly the user can press play and the game will start
...
def newuser(username,password,player,message):#Function make a new user, inputting it onto the database
    un=username.get()
    pw=password.get()
    indb=True

    if len(user)!=2:
        #Validation for username a password so that the username and password is an acceptable length
        if len(un)<3:
...
def player():#This creates the tkinter login page using grid
    global correct
    ...
    #These are buttons that use data from the entries, using functions to log in and make new users
...
while  correct== False:
    master=tk.Tk()
    player()#Starts the login and gets the player usernames to be used later in the program
